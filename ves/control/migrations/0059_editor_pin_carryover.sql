-- =====================================================================
-- 0059_editor_pin_carryover.sql — '시각 고정' 자막을 승계가 버리지 않게 (V3-b)
-- 적용 순서: DB 먼저 무해 — 구 대시보드에는 개선(신규 줄도 종전엔 같은 구멍으로
-- 구간-만 재제출에서 사라졌다), 신 대시보드(pin)에는 필수.
--
-- 델타(0058 전문 기반, 1곳): 구간만 고친 재제출의 자막 승계 필터(s ? 'source_time_sec')
-- 제거 — 앵커 없는 줄(신규·시각 고정)은 사람이 정한 값이므로 전량 보존한다.
-- =====================================================================
...
VALUES ('orchestrator','0059','claude (0059_editor_pin_carryover.sql 시각 고정 자막 승계 보존)')
