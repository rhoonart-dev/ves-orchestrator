-- 0093 — overlay 체인에서 upload_artifacts 를 뺀다 (2026-08-25, 첫 실전 왕복이 잡았다)
--
-- overlay 잡이 끝난 직후 `upload_artifacts` 가 permanent 실패했다:
--
--     generate 결과(run_id/run_dir) 없음
--
-- 맞는 말이다 — **overlay 파이프라인에는 generate 가 없다**. 그 어댑터는 생성 런의
-- run_dir 에서 shorts.mp4·preview 를 올리는 일을 하는데, 외부 완성본에는 그런 것이
-- 애초에 없다. 체인을 KR 생성 체인에서 베껴 오며 딸려 온 단계다.
--
-- 올릴 것은 이미 올라간다 — `localize`(overlay) 가 산출을 `ves-localized` 로 올리고
-- 검수함 카드(`localization_qa`)에 `preview_key`·`bucket` 을 실어 세운다. 대시보드가
-- 읽는 것이 그 두 값이다(실측: 카드가 정상 생성됐고 영상이 재생된다).
--
-- ⚠ rerender(롱폼) 체인의 upload_artifacts 는 **그대로 둔다** — 거기엔 generate 가 있다.
