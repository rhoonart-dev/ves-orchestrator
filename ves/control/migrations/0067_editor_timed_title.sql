-- =====================================================================
-- 0067_editor_timed_title.sql — 타임드 제목(E8, ai-video bd58078) 오케스트레이터 파트
-- 계약: p_overrides.title.segments = [{text, start_sec, end_sec}] (편집본 시간축,
-- 겹침·창 검증 정본은 엔진 — 즉시 실패). 대시보드 개방은 editor_e8 플래그.
--
-- ① submit_editor_render (0066 전문 기반, 델타 2):
--    · title.segments 기본 검증(배열·최대 20·text·시각 숫자·end>start) — 형태 오류를
--      렌더에 태우기 전에 사람이 읽을 메시지로 거른다.
--    · v3 스탬프 조건에 title.segments 추가 — **엔진이 v1·v2 스탬프의 segments 를
--      즉시 거절**하므로, 제목 세그먼트만 고친 제출이 v1 로 찍히면 렌더가 죽는다.
-- ② request_editor_assets (0057 전문 기반, 델타 1): 반환에 prev_title_segments —
--    라운드 승계가 title 키를 통째 교체하므로, 이전 세그먼트가 화면에 보여야
--    재제출이 그 값을 실어 보존한다(prev_images 와 같은 이유 — 0057).
-- 적용 순서: DB 먼저 무해(구 대시보드는 segments 를 안 보낸다).
-- =====================================================================
...
VALUES ('orchestrator','0067','claude (0067_editor_timed_title.sql 타임드 제목 E8 오케스트레이터 파트)')
