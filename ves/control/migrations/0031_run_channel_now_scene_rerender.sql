-- =====================================================================
-- 0031_run_channel_now_scene_rerender.sql — 관제 '작업 실행'을 scene_rerender 체인으로
--                                            (컷오버, 2026-08-13 사용자 결정)
--
-- planner.job_chain 이 JP 채널 localize 를 scene_rerender(생성 노드 재렌더)로 바꿨다.
-- run_channel_now 가 옛 체인(등급 J·캡 localize·ttl 3600)을 계속 만들면 같은 일이
-- 두 곳에서 다르게 돌아간다 — tests/test_pure.py::test_manual_run_chain_matches_planner
-- 가 이 파일(마지막 재정의)을 본다. 0029 대비 바뀐 것 세 가지:
--   ① v_gen: 등급 J 의 '내용만 생성' 플래그 4종 제거 — JP 채널도 완전 렌더.
--      (재렌더가 체크포인트에서 일본어판을 새로 그리므로 처음부터 안 그릴 이유가 없다)
--   ② v_loc: level/backend/voice_id 대신 {'mode':'scene_rerender'} 하나.
--      (등급 경로는 어댑터에 남아 있다 — zanmang_daily 등 완성-mp4 파이프라인 전용)
--   ③ localize 스텝: 캡 ['localize'](mm-06) → ['generate'] · ttl 3600 → 300.
--      generate 완료 시 executor 가 그 노드로 핀 — job 디렉토리가 거기에만 있다.
-- 소스 선택·레거시 차감·권한 등 나머지는 0029 그대로다.
-- =====================================================================
...
VALUES ('orchestrator','0031','claude (run_channel_now scene_rerender 컷오버 — 캡 generate·ttl 300·mode 파라미터)')
