-- =====================================================================
-- 0049_editor_overrides_carryover.sql — 편집 라운드 누적 승계 (2026-08-19)
--
...
-- from_step 도 병합 결과 기준 — 승계된 clips/tts 가 있으면 resources 부터
-- (mp3 재합성·구간 재추출이 그 단계다).
-- 스키마 스탬프도 병합 결과 기준: tts 가 있으면 v2, 없으면 v1 (0047 규약 그대로).
--
-- 짝: dashboard 편집실 edCollect(더티 키만 전송) · ves/adapters/aivideo.py ·
--     ai-video app/modules/edit_overrides.py. 본문은 0047 판 + 위 델타.
-- =====================================================================
...
DECLARE
    ...
    v_prev jsonb; v_ov jsonb; v_step text;
    v_params jsonb; v_common jsonb; v_acq_params jsonb;
...
VALUES ('orchestrator','0049','claude (편집 라운드 누적 승계 — 이전 edit_overrides 키 단위 병합)')
