...
--
-- ⚠ 적용 순서: **DB 적용이 먼저고, 노드가 따라온다** — 역순은 불가능하다. updater 의
-- 마이그레이션 게이트(ves/agent/updater.py ★③ gate_blocks)가 '새 코드 + 구 스키마'를
-- 막으므로, 이 파일이 applied_migrations 에 오르기 전에는 노드가 0048 포함 sha 로 못
-- 올라간다. 적용되면 게이트가 열리고 version_watch(시간당 1회)가 last_seen_sha 를 갱신,
-- 각 노드는 claim 직전 자동 갱신된다. 그 사이 창(대략 1시간 이내)에는 구 어댑터 노드가
-- scan_gen 없는 재료를 다시 써서(coalesce→1) 편집실을 열 때마다 재생성될 수 있다 —
-- 유한하고 자가 치유되는 churn 이니 경보로 오인하지 말 것. (2026-08-19 운영 적용도 이 순서.)
--
