-- 0090 — 드라이브 소재의 acquire 는 인증이 있는 노드에서 (L-P5-3b)
--
-- 실측(2026-08-25): rclone.conf 는 **mm-01·mm-02**, 현지화 스택(localize 캡)은 **mm-06**.
-- 다른 기계다. 그래서 드라이브 파일을 받는 acquire 잡에는 노드 핀을 붙인다 — 안 붙이면
-- 아무 노드나 집어 '인증 없음'으로 죽는다(그리고 재시도해도 같은 자리다).
--
-- 유튜브발은 아무 노드나 받는다(yt-dlp 는 어디에나 있다).
-- ⚠ 파일이 기계 사이를 건너므로 acquire 가 ves-sources 에 올리고 localize 가 내린다.
--   나중에 mm-06 에 rclone.conf 를 두면 이 왕복을 없앨 수 있다(별건).
...
BEGIN
    ...
    -- 🛑 드라이브 파일은 **rclone 인증이 있는 노드**에서만 받는다(실측 2026-08-25:
    --    인증은 mm-01·mm-02, 현지화 스택은 mm-06 — 다른 기계다). 캡을 안 붙이면 아무
    --    노드나 집어 '인증 없음'으로 죽는다. 유튜브발은 아무 노드나 받는다.
    v_acq_caps := CASE
        WHEN v_es.flags->>'drive_file_id' IS NOT NULL
             AND coalesce((SELECT value FROM public.ops_config
                            WHERE key='drive_sync_node'), '') <> ''
        THEN ARRAY['network', 'node:' || (SELECT value FROM public.ops_config
                                           WHERE key='drive_sync_node')]
        ELSE ARRAY['network'] END;
