...
⚠ **남은 것 둘.** 이 판으로 덮이지 않았다:
- **ArcFace 임베딩** — 그 job 에 `checkpoint_research.json` 이 없어 대조 자체가 없었다.
  deepface 는 P0 §2-6 이 지목한 두 축 중 하나다. 리서치가 있는 job 으로 한 판 더 뜬다.
- **표본 1편 · 클립 2개.** 결정적 검출이라 표본이 커도 결과가 같을 공산이 크지만,
