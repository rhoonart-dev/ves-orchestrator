...
## 교정 (8/21 발주 검수 후, ai-video `9faa4fe`)

검수 지적 수용: 후속 2·3(4bd8528·ac67a90)은 video_y 만 쓰는 운영 채널(한 입 주막
13:9·video_y=440·tts 550 — 2026-08-19 사람이 실렌더로 픽셀 튜닝)의 자막을 조용히
움직였다(대사 597→744, TTS 550→697). 회귀 0 원칙 위반이라 게이트를 넣었다:

- **자막·TTS margin 의 밴드 하단 앵커는 `design.video_width` 가 명시된 경우에만**
  (1080 포함). 미지정이면 대사(세로 중앙 가정)·TTS(절대 tts_line_y_margin) 모두
  종전과 동일 — DesignConfig.video_width 기본을 None(미지정 센티널)으로 바꿔
  '명시한 1080'과 '미지정'을 구분한다. 렌더 필터그래프는 두 경우 바이트 동일.
- 실측 margin (HANIPJUMAK 형상, 테스트로 고정): 미지정 → 대사 **597**·TTS **550**
  (종전 그대로) / video_width=1080 명시 → 대사 **744**·TTS **697** (신 기하).
- 위 "발주 범위 밖 후속 3건" 절의 미리보기 미러 조건도 동일하게 'video_width
  명시 시에만'으로 읽어라. 전체 520 테스트 통과.
