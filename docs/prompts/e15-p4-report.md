...
## 5-2c. 실측 3 — 더빙 (2026-08-25 · `G-mRZ8ncYp8`) — **구조 동등**

⚠ `5b2NhVS2h_o` 는 **대사 0건**이라 더빙 소재가 못 된다. 구·신 **둘 다** `dub_from_video`
같은 자리에서 `받아쓰기된 대사 없음` 으로 실패했다 — 그 자체가 ASR 경로 일치의 증거다.
대사 18건짜리로 다시 붙였다.
...
| 산출 | 3개 · 31M | 3개 · 31M |
| **`final_dubbed.mp4`** | **37.453991s** | **37.453991s** |
| 소요 | 2:00 | 2:13 |
...
**이식 결함이 아니다** — 3.6-flash 가 더 장황하게 번역하고, 긴 문장이 슬롯에 안 맞아
배속이 오르고 TTS 백체크가 더 틀린다. 다만 **§8-3 이 CER 을 합격선에 넣은 이유가 이것**
이므로 **실운영 전 사람 청취 검수**가 필요한 항목으로 남긴다. (모델 규칙이 3.5-flash 를
금지하므로 되돌리는 선택지는 없다.)

⚠ **self-ref 프로브는 이번에도 안 돌았다** — `_SELF_MODULE` 로 고친 경로는 미검증이다.
...
| | 무엇 | 막고 있는 것 |
|---|---|---|
| a | 실측 — **BJ·C·더빙 통과**(§5-2·§5-2b·§5-2c). 남은 것: route BC · self-ref 프로브 · 프레임 픽셀 대조 | 소재 |
| a2 | **더빙 품질 사람 청취 검수** — 3.6-flash 가 더 장황해 배속·CER 이 나빠졌다(§5-2c) | 사람 |
