...
## 5-2b. 실측 2 — route C (2026-08-25) — **인페인팅 경로까지 회귀 0**

route C 는 **기준본이 없었다** — 그 소재의 autopilot 산출은 BJ 였다. 구 엔진도 C 로 한 번
돌려 기준을 만들고 대조했다. ⚠ `--video-id` 를 반드시 바꾼다(같은 id 로 돌리면 autopilot
운영 산출물을 덮어쓴다).
...
**이식본의 절반이 이번에 처음 돌았다.** BJ 는 `mask`·`inpaint`·`render(replace)`·FFV1
재조립을 전부 건너뛴다. route C 가 그 넷을 태웠고:

| | 구 | 신 |
|---|---|---|
| `lama` 인페인팅 | 337/337 · 18분 17초 | 337/337 · 18분 09초 |
| 렌더 백체크 | 22건 검사 · 일치 15 · CER 0.318 | **글자까지 동일** |

렌더 백체크는 인페인팅한 프레임 위에 일본어를 그린 뒤 **다시 OCR 해 자가 검증**한
값이다 — `overlay_ab` 가 프레임 픽셀을 안 보는 한계를 이 숫자가 상당 부분 메운다.

⚠ **번역 18/18 동일은 재료가 약하다는 증거이기도 하다.** 구는 `gemini-3.5-flash`, 신은
`gemini-3.6-flash` 인데 결과가 같은 이유는 입력이 거의 전부 기호·단일 문자라 두 모델 다
'그대로 유지'로 답했기 때문이다. **번역 품질 회귀는 두 실측 모두에서 못 봤다.**

### 🛑 실런이 드러낸 운영 성질 2건 (이식 결함 아님 · 개선 후보)

1. **비싼 단계 뒤에 자격증명 검사가 있다.** `ves.env` 의 `GEMINI_API_KEY` 는 수명이
   짧다(실측: 106자 → 만료 → 갱신 후 53자). **18분짜리 인페인팅을 다 하고 나서** 번역이
   `401 UNAUTHENTICATED` 로 죽었다 — 비싼 단계 **앞**에서 키를 확인하면 그 18분을 안
   버린다. vlp 도 같은 구조라 이식 결함은 아니고, 지금 고치면 이식 대조가 흔들리므로
   **별건**이다. ⚠ 재개해도 인페인팅은 다시 돈다(`inpaint_sequence` 가 기존 결과를 안
   건너뛴다) — 실측에서는 모듈을 직접 불러 번역부터 재개해 18분을 아꼈다.
2. **JP 폰트가 양쪽 다 없다.** `NotoSansJP-Black.otf` 를 vlp(`fonts/`)도 이식본
   (`app/assets/fonts/`)도 못 찾아 기본 폰트로 떨어진다. 같은 조건이라 대조는 유효했지만
   **운영 발행 전에 채워야 한다.**
...
| | 무엇 | 막고 있는 것 |
|---|---|---|
| a | 실측 회귀 0 — **route BJ·C 통과**(§5-2·§5-2b). 남은 것: **더빙**(`dub.py`)·route BC·나머지 8편 | 소재(autopilot 산출이 3건뿐) |
