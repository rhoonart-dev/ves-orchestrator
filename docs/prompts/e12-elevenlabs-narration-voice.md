...
### ⑤ 선행 결과 — **미리듣기 경로** 실측 (2026-08-23, 사용자)

편집실 즉석 미리듣기(엣지 함수 `tts-preview`)로 한 줄을 들어본 결과: **소리가 나오고
한국어 발음·억양이 자연스럽다**. E12-0 ② 가 경계한 실패(영어권 목소리가 한국어를 읽어
억양이 배어난다)는 2026-08-22 에 보이스 라이브러리에서 계정에 담은 **한국어 네이티브
12종**(`ops_config.editor_tts_voices`, 목록 32종)으로 비켜 갔다. 부수 확인:
`ELEVENLABS_API_KEY` 가 유효하다 — 존재만이 아니라 실제 합성이 됐다.

⚠ **이것으로 ⑤ 가 닫히지는 않는다.** 이 실측은 **엣지 함수 경로**이지 엔진 렌더 경로가
아니다. 둘은 같은 파라미터를 쓰도록 맞춰 놨을 뿐(모델·`voice_settings`·speed 매핑)
아직 같은 소리라는 증거가 없다 — 증거는 **같은 목소리로 한 편을 재렌더해 완성본 소리가
미리듣기와 같은지**다. 다르면 엔진의 실제 값이 다른 것이니 그 값을 보고해라(고칠 자리는
화면 `ED_EL_SPEED_FACTOR` 와 엣지 함수 `EL_SPEED` 두 상수뿐이고, 테스트가 둘의 일치를
검사한다). ①~④ 는 그대로 엔진 몫이다.
