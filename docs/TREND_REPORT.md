...
### △ 정정 (2026-08-27 오후 — 성과 검증 3차 '4개월 늦은 1화' 반영)

위 판정("출생 시점 억제")은 **기각됐다.** 외부 API 검증: 6월 이후 개설된 외부 신생
채널 10곳 중 9곳이 구독 38~94명으로도 편당 1.8~4.7만 회를 받는다 — 신생·구독 0 은
원인이 아니다. 같은 작품(SNL 시즌8) 외부 29채널 전수에서 편당 1,000회 미만이 0곳이다.

**확정 원인은 콘텐츠다**: ① **회차 지연** — 시즌8 은 3/28 방영인데 우리는 8월에 1~5화를
올린다(1위 채널은 당일 최신 회차, 편당 중앙 395,191회). ② **검색되는 고유명사 부재** —
`#SNL_코리아_리부트_시즌8` 은 검색을 못 탄다. 출연자 실명 태그가 탄다. 락커룸이 터진
날(8/20)도 제목에 실명을 넣은 날이고, 재미쇼츠는 태그가 사라진 8월에 5월 대비 반토막 났다.

처방: **방영 중 작품·최신 회차 우선 배정**(work_cards.start_episode) + **제목·해시태그에
출연자 실명 주입**(파이프라인). 위의 '개별 개설 실험' 권고는 철회한다.

**착수(0101, 8/27)**: 두 처방이 코드로 들어갔다 — `work_cards.cast_names`(발행 시
[작품명]+출연자 실명을 일반 해시태그로 주입, 정산 태그는 별도 경로라 무사) ·
`work_cards.prefer_latest`(planner 소스 정렬 최신 회차 우선). 편집은
`set_work_publish_policy(작품, 출연자[], 최신우선)` RPC. 가왕쇼·SNL 시즌8 시드 완료.
다른 방영작은 사람이 출연자를 채우면 다음 발행부터 적용된다.

덤: LOOPY(잔망루피 JP)는 "관측 불가"가 아니라 **정상 작동 중**이다(총 34,826회) —
laeebly 수집기에 안 잡혔을 뿐. 도달 채널은 3곳이 아니라 4곳.
