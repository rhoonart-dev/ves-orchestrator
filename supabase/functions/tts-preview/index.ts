...
// claim→실행→업로드라 '미리듣기'가 아니다(유휴 폴링 180초). 키가 서버에만 있고 즉시
// 응답하는 자리는 여기뿐이다.
//
...
//
// 짝: dashboard/index.html(edTtsSynth) · ops_config.editor_tts_elevenlabs 게이트
// ─────────────────────────────────────────────────────────────────────────────
...
// ── CORS — 대시보드는 CloudFront(다른 오리진)에서 이 함수를 부른다 ───────────
// supabase-js 의 functions.invoke 는 authorization·apikey·content-type 을 실어 보내므로
// 브라우저가 **본 요청 전에 OPTIONS(프리플라이트)** 를 친다. 그 응답에 허용 헤더가
// 없으면 브라우저가 본 요청을 아예 보내지 않는다 — 함수 로그에는 아무것도 안 남고
// 화면만 "실패"라 원인을 찾기 어렵다(2026-08-23 v2 의 실제 구멍: OPTIONS 가 405 였다).
// 오리진을 * 로 두는 근거: 이 함수는 **호출자의 JWT + reviewer 권한**을 매번 다시 보고,
// 쿠키를 쓰지 않는다(자동으로 실려 가는 자격증명이 없다). 대시보드 도메인이 바뀌어도
// 미리듣기가 조용히 죽지 않는 쪽을 택했다.
